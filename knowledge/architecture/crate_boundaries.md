...
M13 shared synthetic evidence: loader owns generated bytes/report collection, core owns typed session composition, GUI/CLI present session observations. See [GUI evidence](gui_linkage_evidence.md).

M14 loader/artifact/filesystem owns bounded host input acquisition only. Kernel filesystem remains guest VFS/descriptor machinery; this adapter creates no guest file handles or runtime state. See [input boundary](filesystem_input.md).
