use super::{DynamicSymbolObservation, SymbolError};
use crate::{
    elf::{
        ElfInspection,
...
impl<'a> SymbolTable<'a> {
    ...
    pub fn read_candidate(
        ...
        let fields = super::decode::decode(self.elf.artifact().source(), source, index)?;
        let name = if fields.name_offset == 0 {
            ...
            Some(
                self.strings
                    ...
                    .lookup(u64::from(fields.name_offset), max_name_scan_bytes)
